    ...
    def getAction(self, gameState):
        ...
        def value(state, action, depth, index, alpha, beta):
            ...
            if not index:
                ...
                return maximizer(state, depth, index, alpha, beta)
            return minimizer(state, depth, index, alpha, beta)
        ...
        def maximizer(state, depth, index, alpha, beta):
            ...
            for act in acts:
                ...
                val = value(thissucc[0], thissucc[1], depth, ((index + 1) % numagents), alpha, beta)
                v = max(v, val)
                if v > beta:
                    return v
                else:
        ...
        beta = float("inf")
        for a in gameState.getLegalActions(self.index):
            val = value(gameState.generateSuccessor(0, a), a, 0, (self.index + 1), alpha, beta)
            if val > maxval:
                maxval = val
                maxvalaction = a
                alpha = val
